...
def request(url,callback):
    ...
    http_client.fetch(url, callback, proxy_host=proxy_ip["ip"], proxy_port=int(proxy_ip["port"]),request_timeout=60)
    ...
    if response.error:
        ...
        try:
            ...
            for div in divs:
                ...
                e = pq(div)
                name = e('a').attr("title")
                app_id = e('a').attr("href").split("/")[2]
                logger.info("%s, %s, %s, %s ,%s" % (data["type"], data["genre"], rank, app_id, name))
