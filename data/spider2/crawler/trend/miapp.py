...
def handle_app_result(response, app, url, apkname, retry=0):
    ...
    if response.error:
        ...
        try:
            ...
            miapp_parser.process(None, url, apkname, response.body)
...
def begin():
    ...
    while flag is False:
        ...
        # apps = conn.query("select * from artifact where type=4050 and domain ='com.sankuai.meituan' and id>%s order by id limit 1", cnt)
        ...
        for app in apps:
            ...
            #url = "http://android.miapp.com/miapp/searchAjax.htm?kw=%s" % urllib.quote(app["name"].encode("utf-8"))
            ...
            # total += 1
            # url = "http://app.miapp.cn/apps/public/detail?package_name=%s" % app["domain"]
