...
def has_content(content, offset, ranktitle):
    d = pq(html.fromstring(content.decode("utf-8")))
    title = d('head> title').text().strip()
    if title.find("实时排名")>=0 and content.find("rank-list") >= 0:
        lis = d('div.rank-list> div.thumbnaill')
...
def process(content):
    ...
    r["contents"] = []
    d = pq(html.fromstring(content.decode("utf-8")))
    for li in d('div.rank-list> div.thumbnaill'):
        try:
...
def handle_result(response, data):
    ...
    if response.error:
        ...
        return
    elif has_content(response.body, data["offset"], data["genreName"]) is False:
...
def begin():
    ...
    # t = datetime.datetime.strptime("2017-11-14", "%Y-%m-%d").date()
    ...
    for type in types:
        ...
        for genre in genres:
            ...
            for offset in [0]:
                ...
                data = {"type":type, "genre": genre["id"], "date":t, "offset": offset, "datestr": datestr, "genreName": genre["name"]}
                request(url, lambda r,data=data:handle_result(r, data))
