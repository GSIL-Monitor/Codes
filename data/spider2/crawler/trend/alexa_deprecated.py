...
def begin():
    ...
    while flag is False:
        ...
        apps = conn.query("select * from artifact where type=4010 and domain is not null and (active is null or active='Y') order by id limit %s,1000", cnt)
        if len(apps) <= 0:
            while True:
                if total <= 0:
                    logger.info("Finish.")
