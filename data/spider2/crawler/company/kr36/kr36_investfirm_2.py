...
def process(content, flag):
    ...
    j = json.loads(content)
    # logger.info(content)
    companies = j["data"]["investments"]["data"]
    ttnum = j["data"]["investments"]["totalPages"]
    cnt = 0
    if len(companies) == 0:
        return cnt, ttnum
...
def run(flag, column, listcrawler, companycrawler, concurrent_num, investorId):
    global CURRENT_PAGE
    cnt = 1
    total_page=1
    while True:
        ...
        CURRENT_PAGE += 1
        url = 'https://rong.36kr.com/n/api/org/%s/investment?page=%s' % (investorId, key)
...
def start_run(concurrent_num, flag, investorId):
    global CURRENT_PAGE
    while True:
        logger.info("%s investor id %s %s start...", SOURCE, investorId, flag)
        ...
        for column in columns:
            CURRENT_PAGE = 1
            run(flag, column, listcrawler, companycrawler, concurrent_num, investorId)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        while True:
            logger.info("crawler 36kr portfolio start...")
            # ilists = []
            mongo = db.connect_mongo()
            collection = mongo.investor.checklist
            investors = list(collection.find({"kr36InvestorId": {"$ne": None},"kr36Processed":{"$ne":True}}, limit=1000))
            for investor in investors:
                if investor["kr36InvestorId"] == "None":
                    pass
                else:
                    logger.info("need to crawler 36kr :%s", investor["kr36InvestorId"])
                    # ilists.append(int(investor["kr36InvestorId"]))
                    start_run(1, "incr", int(investor["kr36InvestorId"]))
                collection.update_one({"_id": investor["_id"]}, {"$set": {"kr36Processed": True}})
