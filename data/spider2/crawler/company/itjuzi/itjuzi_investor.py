...
#logger
loghelper.init_logger("crawler_itjuzi_investor", stream=True)
logger = loghelper.get_logger("crawler_itjuzi_investor")
...
def run(g, crawler):
    while True:
        if g.finish(num=25):
            return
        key = g.nextKey()
        url = "https://www.itjuzi.com/investor/%s" % key
        while True:
...
def start_run(concurrent_num, flag):
    while True:
        logger.info("Itjuzi investor %s start...", flag)
        g = GlobalValues.GlobalValues(13030, 36004, flag)

        threads = [gevent.spawn(run, g, ItjuziCrawler()) for i in xrange(concurrent_num)]
        gevent.joinall(threads)

        logger.info("Itjuzi investor %s end.", flag)
