...
def run(g, crawler):
    while True:
        ...
        url = "http://chuangyepu.com/startups/%s" % key
        while True:
...
def start_run(concurrent_num, flag):
    while True:
        logger.info("Chuangyepu company %s start...", flag)
        g = GlobalValues.GlobalValues(13040, 36001, flag, back=10)

        threads = [gevent.spawn(run, g, ItjuziCrawler()) for i in xrange(concurrent_num)]
        gevent.joinall(threads)

        logger.info("Chuangyepu company %s end.", flag)
