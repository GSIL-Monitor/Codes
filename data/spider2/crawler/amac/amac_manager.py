...
#logger
loghelper.init_logger("crawler_amac_manager", stream=True)
logger = loghelper.get_logger("crawler_amac_manager")
...
class CsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        ...
        if title.find("私募基金管理人公示 - 中国基金业协会") >= 0:
            return True
...
def run_cs(crawler):
    while True:
        ...
        crawler_cs(crawler, URL["id"], URL["managerName"])

def crawler_cs(crawler, id, name):

    logger.info("start crawler %s|%s", id, name)
    cs_url = 'http://gs.amac.org.cn/amac-infodisc/res/pof/manager/%s.html' % (id)
...
def run(flag, listcrawler, concurrent_num):
    ...
    while True:
        ...
        url = 'http://gs.amac.org.cn/amac-infodisc/api/pof/manager?rand=0.11843547926810949&page=%s&size=100' % (key)
        data = '{}'
...
def get_total_page(listcrawler):
    url = 'http://gs.amac.org.cn/amac-infodisc/api/pof/manager?rand=0.11843547926810949&page=0&size=100'
    data = '{}'
    headers = {"Content-Type": "application/json"}
    tp = None
    while True:
