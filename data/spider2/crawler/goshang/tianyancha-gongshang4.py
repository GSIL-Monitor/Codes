...
#logger
loghelper.init_logger("tianyancha_gongshang3", stream=True)
    ...
    while True:
        ...
        try:
            ...
            if proxy is not None:
                logger.info("worker: %s, %s"% (worker,proxy))
    ...
    while True:
        try:
            ...
            if retries < 3:
                ...
                traceback.print_exc()
                logger.info("worker: %s, fetch error! proxy: %s" % (local.worker_no, proxy))
        break
    ...
    if response is not None:
        if response.status_code != 200:
            if response.status_code == 405:
                ...
                logger.info("worker: %s, Error: %s, %s" % (local.worker_no, response.status_code, url))
                logger.info("worker: %s, proxy: %s" % (local.worker_no, proxy))
        else:
            return response
...
def step1(proxy, company_name):
    ...
    if TYCID is not None:
        logger.info("worker: %s, step1: TYCID=%s" % (local.worker_no, TYCID))
        mongo = db.connect_mongo()
        proxy["TYCID"] = TYCID
    ...
    logger.info("worker: %s, step1: can't get TYCID" % local.worker_no)
    return False
...
def step2(proxy, company_name):
    ...
    if token is None or _utm is None:
        logger.info("worker: %s, %s" % (local.worker_no, html))
        logger.info("worker: %s, step2: can't get token,_utm" % local.worker_no)
        return False
    ...
    mongo.close()
    logger.info("worker: %s, step2: TYCID=%s, token=%s, _utm=%s" % (local.worker_no, TYCID, token, _utm))
    return True
...
def step3(proxy, company_name):
    ...
    if status == -1:
        ...
        logger.info("worker: %s, %s Not Found!" % (local.worker_no, company_name))
        return 0

    logger.info("worker: %s, step3: tyc_company_id=%s" % (local.worker_no, tyc_company_id))
...
def step5(proxy, company_name, tyc_company_id):
    ...
    mongo.raw.proxy_tyc.update({"_id": proxy["_id"]}, {"$set": {"token": token, "utm": _utm}})
    mongo.close()
    logger.info("worker: %s, step5: TYCID=%s, token=%s, _utm=%s" % (local.worker_no, proxy["TYCID"], token, _utm))
...
def step6(proxy, company_name, tyc_company_id):
    ...
    try:
        # html = unicode(response.text, encoding="utf-8", errors='replace')
        html = response.text
        logger.info("worker: %s, step6: " % local.worker_no)
        logger.info("worker: %s, %s" % (local.worker_no, html))
    except:
        traceback.print_exc()
    ...
    logger.info("worker: %s, step6: success" % local.worker_no)
...
def worker(q, lock, index):
    local.worker_no = index
    proxy = get_proxy(lock, index)
    while True:
        try:
            company = q.get()
            if "/" in company["name"] or "." in company["name"]:
                update_check_time(company, False)
            else:
                logger.info('worker: %s, fetching: %s' % (local.worker_no, company["name"]))
                while True:
    ...
    while True:
        if q.empty():
            ...
            cs = list(mongo.info.company_idx.find({
                    "$or":[
                        {"tycCheckTime": None},
                        {"tycCheckTime": {"$lt":datetime.datetime.now()-datetime.timedelta(days=20)}}
                    ]
            }).limit(concurrency*5))
            mongo.close()
...
if __name__ == '__main__':
    ...
    for index in range(concurrency):
        w = threading.Thread(target=worker, args=(q, lock, index))
        w.start()
    main(q)
