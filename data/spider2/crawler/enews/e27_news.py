...
#logger
loghelper.init_logger("crawler_e27_news", stream=True)
logger = loghelper.get_logger("crawler_e27_news")
...
def process_news(column, newsurl, content, newspost, topic, download_crawler):
    if has_news_content(content):
        ...
        title = d('h1.post-heading').text().strip()
        ...
        article = d('div.post-content').html()
...
def process(content, flag):
    if content.find("e27") >= 0:
        ...
        for a in d('div.col-md-8.content'):
            try:
...
def run(flag, column, listcrawler, newscrawler, concurrent_num):
    ...
    while True:
        ...
        url = "https://e27.co/category/%s?json" % column["column"]
