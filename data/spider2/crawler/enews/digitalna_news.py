...
#logger
loghelper.init_logger("crawler_digitalnewsasia_news", stream=True)
logger = loghelper.get_logger("crawler_digitalnewsasia_news")

NEWSSOURCE = "digitalnewsasia"
RETRY = 3
TYPE = 60004
SOURCE =13885
URLS = []
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self,url,content):
        ...
        if title.find("Digital News Asia") >= 0:
            return True
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self,url,content):
        ...
        title = d('head> title').text().strip()
        logger.info("title: %s url: %s", title, url)
        if content.find("Digital News Asia") >= 0:
            return True
...
def process_news(column, newsurl, content, newspost, topic, download_crawler):
    if has_news_content(content):
        ...
        title = d('h1.page-header').text().strip()
        ...
        try:
           post_time = d("meta[property='article:published_time']").attr("content")
        ...
        article = d('div.content> div.field-name-body> div.field-items> div.field-item').html()
...
def process(content, flag):
    if content.find("digitalnewsasia") >= 0:
        # logger.info("aaa")
        d = pq(html.fromstring(content.decode("utf-8")))
        for a in d('div.mininews-news> h3'):
            try:
                ...
                if re.search(linkPattern, link) and title is not None and title.strip() != "":
                    ...
                    linknew = "https://www.digitalnewsasia.com" + link
