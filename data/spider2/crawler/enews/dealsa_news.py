...
#logger
loghelper.init_logger("crawler_dealstreetasia_news", stream=True)
logger = loghelper.get_logger("crawler_dealstreetasia_news")

NEWSSOURCE = "dealstreetasia"
RETRY = 3
TYPE = 60004
SOURCE =13884
URLS = []
CURRENT_PAGE = 1
linkPattern = "www.dealstreetasia.com/\w+"
Nocontents = [
]
columns = [
    # {"column": "jmd", "max": 2},
    {"column": "china-hk", "max": 1},
    {"column": "singapore", "max": 1},
    {"column": "indonesia", "max": 1},
    {"column": "philippines", "max": 1},
    {"column": "myanmar", "max": 1},
    {"column": "thailand", "max": 1},
    {"column": "vietnam", "max": 1},
    {"column": "malaysia", "max": 1},
    {"column": "india", "max": 1},
    {"column": "international", "max": 1},
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self,url,content):
        ...
        if title.find("DealStreetAsia") >= 0:
            return True
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self,url,content):
        ...
        title = d('head> title').text().strip()
        logger.info("title: %s url: %s", title, url)
        if content.find("dealstreetasia") >= 0:
            return True
...
def process(content, flag):
    if content.find("dealstreetasia") >= 0:
        ...
        for a in d('div#main> div.row'):
            try:
                link = d(a)('h1> a, h3> a').attr("href")
                title = d(a)('h1> a, h3> a').text()
                ndate = None
                logger.info(link)
...
def run(flag, column, listcrawler, newscrawler, concurrent_num):
    ...
    while True:
        ...
        url = "https://www.dealstreetasia.com/countries/%s/" % column["column"]
