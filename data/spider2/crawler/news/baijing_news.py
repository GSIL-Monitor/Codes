...
#logger
loghelper.init_logger("crawler_baijing_news", stream=True)
logger = loghelper.get_logger("crawler_baijing_news")
...
URLS = []
CURRENT_PAGE = 1
linkPattern = "www.baijingapp.com/article/\d+"
Nocontents = [
]
columns = [
...
def process(content, flag):
    if content.find("baijing") >= 0:
        d = pq(html.fromstring(content.decode("utf-8")))
        for a in d('div.aw-article-list> div.articleSingle'):
            try:
                link = d(a)('div.articleSingle-info> h1> a').attr("href")
                title = d(a)('div.articleSingle-info> h1> a').text()
                # logger.info(link)
                if re.search(linkPattern, link) and title is not None and title.strip() != "":
                    # logger.info("Link: %s is right news link %s", link, title)
                    # title = d(a)('h3> a').text()
                    post = d(a)('div.articleSingle-img> a.post-thumb> img').attr("src")
                    sort = d(a)('div.articleSingle-img> a.sort').text()
