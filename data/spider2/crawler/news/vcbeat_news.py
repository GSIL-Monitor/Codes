...
# logger
loghelper.init_logger("crawler_vcbeat_news", stream=True)
logger = loghelper.get_logger("crawler_vcbeat_news")
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    def __init__(self):
        ...
        def is_crawl_success(self, url, content):
            if content.find('<div class="art_text">') >= 0:
                return True
            return False
...
def process(content, flag):
    ...
    for a in htmls:
        title = a['title']
        link = 'http://vcbeat.net/' + a['detail_id']
        post = 'http://vcbeat.net' + a['logo_path']
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = 'http://vcbeat.net/Index/Index/ajaxGetArticleList?page=%s&categoryId=0' % (key)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # crawler_news({"column": None, "max": 3}, NewsCrawler(), 'http://vcbeat.com/Xfeature/view?aid=1603', None, download.DownloadCrawler(use_proxy=False))
