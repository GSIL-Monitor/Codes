...
#logger
loghelper.init_logger("crawler_donews_news", stream=True)
logger = loghelper.get_logger("crawler_donews_news")
...
URLS = []
CURRENT_PAGE = 1
linkPattern = "www.donews.com/news/detail/\d/\d+.html"
Nocontents = [
]
columns = [
...
def process_news(column, newsurl, content, newspost, download_crawler):
    if has_news_content(content):
        ...
        type = TYPE
        if column["column"] == "article":
            type = 60005
        category = None

        title = d('div.contentbox> h2').eq(0).text().strip()
        ...
        try:
            post_time =d('div.tag> p> span').eq(1).text()
            ...
            news_time = datetime.datetime.now()
        article = d('div.article-con').html()
...
def process(content, flag):
    if content.find("donews") >= 0:
        d = pq(html.fromstring(content.decode("utf-8")))
        for a in d('div.block> dl.block'):
            try:
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = "http://www.donews.com/%s/index" % column["column"]
        if column["column"] == "article":
            url = "http://www.donews.com/article/show_recommend"
