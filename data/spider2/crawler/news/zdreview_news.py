...
#logger
loghelper.init_logger("crawler_zdreview_news", stream=True)
logger = loghelper.get_logger("crawler_zdreview_news")
...
URLS = []
CURRENT_PAGE = 1
linkPattern = "zdreview.com/archives/\d"
Nocontents = [
]
columns = [
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self,url,content):
        ...
        if title.find("竺道") >= 0:
            return True
        if title.find("Nothing found for") >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler, sort):
    if has_news_content(content):
        ...
        title = d('div.m-sd-post-box> h1').text().strip()
        ...
        article = d('div.g-post-content').html()
        contents = extract.extractContents(newsurl, article, document=False)

        logger.info("%s, %s, %s, %s -> %s, %s, %s, %s", key, title, news_time, ":".join(tags),type, category, brief, post)
        # exit()
...
def process(content, flag):
    if content.find("zdreview") >= 0:
        d = pq(html.fromstring(content.decode("utf-8")))
        for a in d('ul.nice-ajax-con> li'):
            try:
                link = d(a)('div.content> h2> a').attr("href")
                title = d(a)('div.content> h2> a').text()
                # logger.info(link)
                if re.search(linkPattern, link) and title is not None and title.strip() != "":
                    # logger.info("Link: %s is right news link %s", link, title)
                    # title = d(a)('h3> a').text()
                    post = None
                    sort = d(a)('div.image> span.g-ig-mark').text()
