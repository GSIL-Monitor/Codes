...
#logger
loghelper.init_logger("crawler_sanwenyu_news", stream=True)
logger = loghelper.get_logger("crawler_sanwenyu_news")
...
URLS = []
CURRENT_PAGE = 1
linkPattern = "www.3wyu.com/\d+.html"
Nocontents = [
]
columns = [
    {"column": "new", "max": 1},
...
def process(content, flag):
    if content.find("3wyu") >= 0:
        d = pq(html.fromstring(content.decode("utf-8")))
        # for li in d('div.artile_list> ul> li'):
        for li in d('div.swy-jrkd> dl'):
            try:
                # link = d(li)('div.t> a').attr("href").strip()
                link = d(li)('h2> a').attr("href").strip()
                # logger.info(link)
                if re.search(linkPattern, link):
                    logger.info("Link: %s is right news link", link)
                    # title = d(li)('div.t> a').text()
                    # logger.info("title: %s", title)
                    # post = d(li)('div.i> a> img').attr("src")
                    title = d(li)('h2> a').text()
                    logger.info("title: %s", title)
                    post = d(li)('dt> a> img').attr("src")
