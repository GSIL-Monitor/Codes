...
# logger
loghelper.init_logger("crawler_pedata_news", stream=True)
logger = loghelper.get_logger("crawler_pedata_news")
    ...
    def __init__(self, timeout=30):
        ...
        def is_crawl_success(self, url, content):
            if content is not None:
                if content.find('<div class="new_news_one">') >= 0:
                    d = pq(html.fromstring(content.decode("utf-8")))
                    htmls = d('.new_news_one')
                    if len(htmls) > 0:
                        return True
                    else:
                        return False
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content.find('<div class="article_main">') >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    ...
    if 1:
        ...
        if title is None or title == '':
            logger.info('title is None, check!!!')
            exit()
        ...
        # if post is not None:
        #     post = "http://pedata.com"+ post

        brief = d(".article_zy").text()
        ...
        news_time = datetime.datetime.strptime(d(".article_time").text(), '%Y-%m-%d %H:%M:%S')
        # dt = datetime.date.today()
        # today = datetime.datetime.now()
        # if news_time is None or news_time > today:
        #     news_time = datetime.datetime.now()

        article = d('.article_main').html()
...
def crawler_news(column, crawler, newsurl, newspost, download_crawler):
    ...
    while True:
        if retries > 10: break
        retries += 1
        headers = {"Cache-Control": "max-age=0", "Referer": "http://news.pedata.cn/"}
...
def process(content, flag):
    ...
    cnt = 0
    if len(htmls) == 0:
        logger.info('the page got 0 items, check')
        return cnt
    for a in htmls:
        title = d(a)('h2').text().strip()
        link = d(a)("h2 a").attr('href')
        post = d(a)("img").attr('src')
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = 'http://news.pedata.cn/list_%s_0.html' % (key)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # crawler_news({"column": None, "max": 3}, NewsCrawler(), 'http://pedata.com/Xfeature/view?aid=1603', None, download.DownloadCrawler(use_proxy=False))
