...
# logger
loghelper.init_logger("crawler_technode_news", stream=True)
logger = loghelper.get_logger("crawler_technode_news")
...
# CURRENT_PAGE = 1
#
linkPattern = "https://cn.technode.com/post/.*"
# Nocontents = [
# ]
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content is not None:
            if content.find('span8 column_container') >= 0:
                return True
            return False
        return False
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        ...
        if title.find("动点科技") >= 0 or content.find("<article>"):
            return True
        return False

def has_news_content(content):
    d = pq(html.fromstring(content.decode("utf-8")))
    if d('article').attr('id').find('post') >= 0:
        return True
    return False
...
def process_news(newsurl,content,newspost,download_crawler):
    if has_news_content(content):
        ...
        try:
            post_time = d("header> div> time").text().strip()
            res = re.search('(\d{4})/(\d{2})/(\d{2})',post_time)
            post_time = '{}-{}-{}'.format(res.group(1),res.group(2),res.group(3))
...
def process(content, flag):
    if content.find("td_mod_wrap td_mod9") >= 0:
        d = pq(html.fromstring(content.decode('utf-8','ignore')))
        for lis in d('div.td_mod9'):
            try:
...
def run(flag, listcrawler, newscrawler,  download_crawler):
    cnt = 1
    while True:
        url = "https://cn.technode.com/post/category/news/"
        if cnt == 0:
            return
        while True:
