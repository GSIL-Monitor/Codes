...
#logger
loghelper.init_logger("crawler_datayuan_news", stream=True)
logger = loghelper.get_logger("crawler_datayuan_news")
...
def process_news(column, newsurl, content, newspost, topic, download_crawler):
    if has_news_content(content):
        ...
        title1 = d('head> title').text().strip()
        if title1.find("融资") >= 0:
            type = 60001
            category = 60101
        ...
        title = d('div.wz-div > h1').text().strip()
        ...
        article = d('div.wz-div-text').html()
...
def process(content, flag):
    if content.find("datayuan") >= 0:
        d = pq(html.fromstring(content.decode("utf-8")))
        for a in d('div.main-div-l> div> div.wz-div'):
            try:
