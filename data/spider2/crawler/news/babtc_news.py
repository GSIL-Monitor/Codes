...
#logger
loghelper.init_logger("crawler_8btc_news", stream=True)
logger = loghelper.get_logger("crawler_8btc_news")
...
class ListCrawler(BaseCrawler.BaseCrawler):
    def __init__(self,timeout=40,use_proxy=False):
...
def process_news(column, newsurl, content, newspost, topic, download_crawler):
    if has_news_content(content):
        ...
        try:
           post_time =d("span.header__info-item").text().split(" ")[1].replace("T"," ")
...
def process(content, flag):
    if content.find("8btc") >= 0:
        d = pq(html.fromstring(content.decode("utf-8","ignore")))
        for a in d('div.article-item> div> a'):
            try:
                link = d(a)('a.title').attr("href")
                title = d(a)('a.title').text()
                link = "https://www.8btc.com" + link
