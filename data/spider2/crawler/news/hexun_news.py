...
# logger
loghelper.init_logger("crawler_hexun_news", stream=True)
logger = loghelper.get_logger("crawler_hexun_news")
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    # 实现
    def is_crawl_success(self, url, content):
        if content.find("art_contextBox") > -1:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, topic, download_crawler):
    ...
    if 1:
        ...
        title = d('.articleName h1').text().strip()
...
def run(flag, column, listcrawler, newscrawler, concurrent_num):
    ...
    while True:
        ...
        url = "http://roll.hexun.com/roolNews_listRool.action?type=all&date=%s&page=%s" % (today, key)
