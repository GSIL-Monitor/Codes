...
# logger
loghelper.init_logger("crawler_pedailynf_news", stream=True)
logger = loghelper.get_logger("crawler_pedailynf_news")

NEWSSOURCE = "Pedaily_NEWFEED"
RETRY = 3
TYPE = 60001
SOURCE = 13842
URLS = []
CURRENT_PAGE = 1
linkPattern = "http://news.newseed.cn/p/\d+"
Nocontents = [
]
columns = [
    {"column": "new", "max": 3},
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        ...
        if content.find('<div class="news-list" id="news-list">') >= 0:
            return True
        return False
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    # 实现
    def is_crawl_success(self, url, content):
        if content.find('<div id="news-content" class="news-content">') >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    if has_news_content(content):
        ...
        #     title = d('div.box-r> div> h1').text().strip()

        if title is None or title == "":
            return
        tags = []
        articletags = d(".news-tags a")
        if articletags is not None:
            for tag in articletags:
                tag = d(tag).text().strip()
        ...
        brief = d(".subject").text().strip()

        post_time = d('.news-show-title .date').text()
        logger.info(post_time)
        news_time = extract.extracttime(post_time)
        if news_time is None:
            news_time = datetime.datetime.now()

        article = d('.news-content').html()
...
def process(content, flag):
    d = pq(html.fromstring(content.decode("utf-8")))
    for li in d('.news-list li'):
        try:
            href = d(li)('a.title').attr("href").strip()
            link = href
            # logger.info(link)
            if re.search(linkPattern, link):
                logger.info("Link: %s is right news link", link)
                title = d(li)('a.title').text()
                logger.info("title: %s", title)
                post = d(li)('div.img> a> img').attr("data-src")
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = "http://news.newseed.cn/p%s" % (key)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if param == "incr":
            ...
            download_crawler = download.DownloadCrawler(use_proxy=False)
            # download_crawler = None
            crawler_news({"column": "new", "max": 3}, ListCrawler(), link,
                         "http://pic2.pedaily.cn/201703/20170328@195829.jpg", download_crawler)
    else:
        start_run(1, "incr")
