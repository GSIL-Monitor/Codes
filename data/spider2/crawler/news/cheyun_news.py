...
# logger
loghelper.init_logger("crawler_cheyun_news", stream=True)
logger = loghelper.get_logger("crawler_cheyun_news")
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content is not None:
            if content.find('<li class="m-wrap-lst') >= 0:
                return True
            return False
        return False
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content.find('<div class="m-con-article') >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    ...
    if 1:
        ...
        title = d('.u-title-article').text().strip()
...
def process(content, flag):
    ...
    if len(htmls) == 0:
        return cnt
    for a in htmls:
        title = d(a)('.title').text().strip()
        link = 'http://www.cheyun.com' + d(a)(".title").attr('href')
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = 'http://www.cheyun.com/content/all/%s' % (key)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # crawler_news({"column": None, "max": 3}, NewsCrawler(), 'http://cheyun.com/Xfeature/view?aid=1603', None, download.DownloadCrawler(use_proxy=False))
