...
#logger
loghelper.init_logger("crawler_morningw_news", stream=True)
logger = loghelper.get_logger("crawler_morningw_news")

NEWSSOURCE = "morningwhistle"
RETRY = 3
TYPE = 60001
SOURCE =13862
URLS = []
CURRENT_PAGE = 1
linkPattern = "www.morningwhistle.com/info/\d.html"
Nocontents = [
]
columns = [
    # {"column": "jmd", "max": 2},
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content is not None:
            ...
            if j.has_key("data") is True and j["data"].has_key("rows") is True:
                return True
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self,url,content):
        ...
        if title.find("晨哨并购") >= 0:
            return True
        if content.find("morningwhistle") >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, topic, download_crawler):
    if has_news_content(content):
        ...
        title = d('div.preview> h1.title').text().strip()
        ...
        brief = d('div.preview> div.introduction').text().strip()

        post_time = d('b.formatDate').attr("data")
        ...
        article = d('div.preview> div.content').html()
...
def process(content, flag):
    ...
    for info in infos:
        try:
            ...
            link = "http://www.morningwhistle.com/" + info["url"]
            post = None
            ...
            if ((item is None and item2 is None) or flag == "all") and link not in URLS and \
                    link.find("liuyong") <0:
                linkmap = {
                    "link": link,
                    "post": post,
...
def run(flag, column, listcrawler, newscrawler, concurrent_num):
    ...
    while True:
        ...
        url = "http://www.morningwhistle.com/website/content/list?industrys=&regions=&tradeTypes=&sort=zx&siteId=8&channelFlag=info&isParent=false&lastIndex=&pageSize=%s&codestr=" % (key*20)
