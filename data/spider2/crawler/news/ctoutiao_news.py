...
# logger
loghelper.init_logger("crawler_ctoutiao_news", stream=True)
logger = loghelper.get_logger("crawler_ctoutiao_news")
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content.find('<div class="A_contxt">') >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    ...
    if 1:
        ...
        tags = []
        articletags = d(".A_linebn a").text().strip()
        ...
        news_time = d('.A_pon1 em').eq(0).text().strip().replace('时间：', '')
        ...
        for c in contents:
            if c["type"] == "text":
                ...
                if download_crawler is None:
                    ...
                    if c["data"].find('grey.gif') >= 0: continue
...
def run_news(column, crawler, download_crawler):
    while True:
        ...
        crawler_news(column, crawler, URL["link"], URL.get('post'), download_crawler)
...
def process(content, flag):
    ...
    d = pq(html.fromstring(j['data']['fetch_data'].decode("utf-8")))
    htmls = d('.newslist')
    ...
    if len(htmls) == 0:
        return cnt
    for a in htmls:
        title = d(a)('h4 a').text().strip()
        link = 'http://www.ctoutiao.com' + d(a)("h4 a").attr('href')
        # post = d(a)(".m-pic-news img").attr('src')
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = 'http://www.ctoutiao.com/ajax_new/ajax_data.php?wap=load&page=cat&act=get_recommend_more&p=%s&cat=NaN' % (
            key)
        headers = {
            'host': "www.ctoutiao.com",
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # crawler_news({"column": None, "max": 3}, NewsCrawler(), 'http://ctoutiao.com/Xfeature/view?aid=1603', None, download.DownloadCrawler(use_proxy=False))
