...
#logger
loghelper.init_logger("crawler_iyiou_newsf", stream=True)
logger = loghelper.get_logger("crawler_iyiou_newsf")
...
CURRENT_PAGE = 1
MAX_PAGE = 2
linkPattern = "\w+.iyiou.com/breaking/\d+"
...
def process(content, flag):
    d = pq(html.fromstring(content.decode("utf-8")))
    for a in d('ul.list_box_zx> li> a'):
        try:
