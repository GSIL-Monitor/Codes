...
#logger
loghelper.init_logger("crawler_weilai_news", stream=True)
logger = loghelper.get_logger("crawler_weilai_news")
...
URLS = []
CURRENT_PAGE = 1
linkPattern = "www.datayuan.cn/\w+/\d+.htm"
Nocontents = [
]
columns = [
    # {"column": "jmd", "max": 2},
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content is not None:
            ...
            if j.has_key("data") is True and j["data"].has_key("article_lists") is True:
                return True
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self,url,content):
        ...
        if title.find("未来财经") >= 0:
            return True
        if title.find("System Error") >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, topic, download_crawler):
    if has_news_content(content):
        ...
        title = d('div.text> h1').text().strip()
        if title.find("融资") >= 0:
            type = 60001
            category = 60101
        ...
        article = d('div.text-content').html()
...
def process(content, flag):
    ...
    for info in infos:
        try:
            ...
            link = "http://www.weilaicaijing.com/article/" + str(key)
            post = info["image"]
            title = info["post_title"].strip()
            ...
            if ((item is None and item2 is None) or flag == "all") and link not in URLS:
                linkmap = {
                    "link": link,
                    "post": str(post),
                    "newsDate": str(info.get("post_date",None))
                }
                URLS.append(linkmap)
        except Exception,ex:
            logger.exception(ex)
...
def run(flag, column, listcrawler, newscrawler, concurrent_num):
    ...
    while True:
        ...
        url = "http://www.weilaicaijing.com/api/Home/article_lists?page=%s&term_taxonomy_id=-1" % key
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if param == "incr":
            ...
            crawler_news({}, NewsCrawler(), link, None, '2018-03-08 11:50:23', download_crawler)
    else:
        start_run(1, "incr")