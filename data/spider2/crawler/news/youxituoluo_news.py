...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        ...
        # logger.info(content)
        if content is None or content.find("article") >= 0 or content.find("游戏陀螺") >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    if has_news_content(content):
        ...
        title = d('div.newshead> h1').text().strip()
        ...
        rank = 1
        for c in contents:
            if c["data"].find("游戏陀螺微信二维码")>=0 or c["data"].find("关注微信公众号：游戏陀螺")>=0:
                break
            if c["type"] == "text":
                dc = {
                    "rank": rank,
...
def process(content, flag):
    if content.find("article") >= 0:
        ...
        for li in d('div.article'):
            try:
                href = d(li)('h1> a').attr("href").strip()
                link = "http://www.youxituoluo.com" + href
                # logger.info(link)
                if re.search(linkPattern, link):
                    logger.info("Link: %s is right news link", link)
                    title = d(li)('h1> a').text()
                    logger.info("title: %s", title)
                    post = "http://www.youxituoluo.com" + d(li)('div.nimg> a> img').attr("src")
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = "http://www.youxituoluo.com/cms/mg.php?r=alltype/more&page=%s" % (key)
