...
# logger
loghelper.init_logger("crawler_luxe_news", stream=True)
logger = loghelper.get_logger("crawler_luxe_news")
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content.find('<div class="post-body') >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    ...
    if 1:
        ...
        TYPE = 60001 if typeNames.find(u'金融') >= 0 or typeNames.find(u'融资') >= 0 else 60005
...
def process(content, flag):
    ...
    if len(htmls) == 0:
        return cnt
    for a in htmls:
        title = d(a)('.title a').text().strip()
        link = d(a)(".title a").attr('href')
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        # if key == 1:
        #     url = 'http://luxe.co/tag/funding'
        # elif key == 2:
        #     url = 'http://luxe.co/category/finance'
        # else:
        #     return
        url = 'http://luxe.co/'
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # crawler_news({"column": None, "max": 3}, NewsCrawler(), 'http://luxe.com/Xfeature/view?aid=1603', None, download.DownloadCrawler(use_proxy=False))
