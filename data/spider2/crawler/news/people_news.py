...
# logger
loghelper.init_logger("crawler_people_news", stream=True)
logger = loghelper.get_logger("crawler_people_news")
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content is not None:
            if content.find('class=" hdNews clearfix"') >= 0:
                return True
            return False
        return False
...
class NewsCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        if content.find('class="gray box_text"') >= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    ...
    if 1:
        ...
        # print content.decode('GB2312','ignore')
        ...
        tags = []
        # articletags = d(".n_s_rd a").text().strip()
        ...
        # if post is not None:
        #     post = "http://people.com"+ post

        brief = None

        # news_time = extractArticlePublishedDate.extractArticlePublishedDate(newsurl, content)
        news_time = d('.author + h2').text().split(u'来源')[0].strip().replace('.0', '')
        news_time = datetime.datetime.strptime(news_time, '%Y年%m月%d日%H:%M')
...
def process(content, flag):
    ...
    # logger.info(content)
    d = pq(html.fromstring(content.decode("GB2312")))
    htmls = d('.hdNews')
    ...
    if len(htmls) == 0:
        return cnt
    for a in htmls:
        title = d(a)('strong a').text().strip()
        link = 'http://capital.people.com.cn' + d(a)("strong a").attr('href')
        post = 'http://capital.people.com.cn/' + d(a)("img").attr('src')
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = 'http://capital.people.com.cn/GB/417685/index%s.html' % (key)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # crawler_news({"column": None, "max": 3}, NewsCrawler(), 'http://people.com/Xfeature/view?aid=1603', None, download.DownloadCrawler(use_proxy=False))
