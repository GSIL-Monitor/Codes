...
#logger
loghelper.init_logger("crawler_pedaily_news", stream=True)
logger = loghelper.get_logger("crawler_pedaily_news")
...
URLS = []
CURRENT_PAGE = 1
linkPattern = "pedaily.cn/\d+/\d+.shtml"
Nocontents = [
]
columns = [
    {"column": "new", "max": 3},
...
class ListCrawler(BaseCrawler.BaseCrawler):
    ...
    def is_crawl_success(self, url, content):
        ...
        if title.find("投资界") >= 0 or title.find("404 Page Not Found")>= 0:
            return True
        return False
...
def process_news(column, newsurl, content, newspost, download_crawler):
    if has_news_content(content):
        ...
        title = d('div.news-show> h1#newstitle').text().strip()
        ...
        post_time = d('div.info> div> span.date').text()
        logger.info(post_time)
        news_time = extract.extracttime(post_time)
        if news_time is None:
            news_time = datetime.datetime.now()

        article = d('div#news-content.news-content').html()
...
def process(content, flag):
    d = pq(html.fromstring(content.decode("utf-8")))
    for li in d('div.news-list> ul> li'):
        try:
            href = d(li)('h3> a.title').attr("href").strip()
            link = href
            # logger.info(link)
            if re.search(linkPattern, link):
                logger.info("Link: %s is right news link", link)
                title = d(li)('h3> a.title').text()
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        if column["column"] == "new":
            url = "http://pe.pedaily.cn/%s/" % (key)
        else:
            url = "http://news.pedaily.cn/%s/" % (key)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if param == "incr":
            ...
            download_crawler = download.DownloadCrawler(use_proxy=False)
            # download_crawler = None
            crawler_news({"column": "new", "max": 3}, ListCrawler(), link, "http://pic2.pedaily.cn/201703/20170328@195829.jpg", download_crawler)
    else:
        start_run(1, "incr")