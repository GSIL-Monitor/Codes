...
#logger
loghelper.init_logger("crawler_Cnstock_news", stream=True)
logger = loghelper.get_logger("crawler_Cnstock_news")
...
URLS = []
CURRENT_PAGE = 1
linkPattern = "http://\w+.cnstock.com/\w+/.*/\d+/\d+.htm"
Nocontents = [
]
columns = [
    {"column": None, "max": 3},
...
def process(content, flag):
    ...
    d = pq(html.fromstring(content.decode("utf-8")))
    for a in d('ul.nf-list> li'):
        try:
            link = d(a)('div.title> a').attr("href").strip()
            # logger.info(link)
...
def run(flag, column, listcrawler, newscrawler, concurrent_num, download_crawler):
    ...
    while True:
        ...
        url = 'http://news.cnstock.com/bwsd/%s' % (key)
