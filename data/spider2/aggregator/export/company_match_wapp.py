...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        # brief = names[1]
        # tag = names[3]
        company_ids = find_company.find_companies_by_full_name_corporate([unicode(fullName)])
        #
        ...
        #     cnt += 1
            ...
            cnt+=1
            c = conn.get("select * from company where id=%s", company_ids[0])
            link = 'http://www.xiniudata.com/validator/#/company/%s/overview' % c["code"]
            # send_message_task(company_ids[0], "company_newcover", 13096)
            line = "%s+++%s+++%s+++%s+++%s\n" % (fullName, c["code"], c["name"], link, c["active"])
            fp2.write(line)
