...
#logger
loghelper.init_logger("refresh_company_artifact", stream=True)
logger = loghelper.get_logger("refresh_company_artifact")
...
def refresh_artifact(company_id):
        # mongo
        mongo = db.connect_mongo()
        # create index?
        ...
        artifacts_and = conn.query(
            "select * from artifact where companyId=%s and (active is null or active='Y') and type=4050",
            company_id)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        tasks2 = list(collection.find({"status": 0, "extendType":1}))
        for t in tasks2:
