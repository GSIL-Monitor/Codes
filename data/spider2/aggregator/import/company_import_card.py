...
def insert_funding(sid,roundstr,inv,fundingDate,investors):
    try:
        inv = "".join(inv.split())
        ...
        source_investors = []
        for investor in investors.replace(",","，").replace("、","，").split("，"):
            if investor is not None and investor.strip() !="":
                source_investor = {
                    "name": investor.strip(),
                    "website": None,
                    "description": None,
        ...
        logger.info(E)
        logger.info("%s/%s/%s/%s", roundstr, inv, fdate, investors)
        # exit()
    pass
