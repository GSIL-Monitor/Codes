        ...
        for source_company_name in source_company_names:
            ...
            if check_name is None:
                if test:
                    ...
                    # 判断 icp_beian_query_by_fullname 失效 条件
                    items_beianlinks = icp_beian_query_by_fullname.main('source_company_name["name"]')
                    # 如果 失效 启用 beian_links_crawler 得到 items_beianlinks
        ...
        for artifact in artifacts:
            ...
            if check_domain is None:
                if test:
                    ...
                    # 判断条件
                    items_beianlinks = icp_beian_query_by_domain.main(domain)
                    # else：
...
#         "select id from source_company limit 1,5")
#     print(source_companies)
#     for source_company in source_companies:
...
#         if company_fullname is not None and company_fullname.strip() != "":
#             company_fullname = name_helper.company_name_normalize(company_fullname)
#             check_fullname = conn.get("select * from source_company_name where sourceCompanyId=%s and name=%s limit 1",
#                                       source_company_id, company_fullname)
