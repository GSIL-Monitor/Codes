...
def aggregate_artifact(company_id,source_company_id, test=False):
    ...
    for sa in sas:
        if sa["active"] == "Y" or sa["active"] is None:
            ...
            if artifact is None and sa["type"] in [4020, 4030]:
                artifact = conn.get("select * from " + table_names["artifact"] + " where companyId=%s and type=%s and name=%s limit 1",
                                    company_id, sa["type"], sa["name"])
            if artifact is None:
