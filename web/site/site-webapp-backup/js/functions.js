...
function appendListSearch(selectedList){
	  var result =  '<div class="list-param label div-tag m-t-sm"  '+
	      'tooltip-placement="top" tooltip="Contains '+selectedList.count+' Companies"'+
	      'select-list="'+selectedList.listId+'">'+
...
function getAllList($scope, $http, $cookieStore, $location){
	...
	$http.get('./api/user/list/getAll').success(function(data){
			$scope.allList = data.userList;
			$scope.total = $scope.allList.length; 
