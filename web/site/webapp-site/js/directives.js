	...
	return {
        ...
        link: function (scope, element, attrs) {
        	element.bind("click", function(event) {
				...
				if(model == 'cf'){
					//	url = './api/site/crowdfunding/getAll';
					url = pre_address+'/api/site/crowdfunding/getAll';
					...
					$http.get(url+'?page='+(++scope.page)+'&status='+status+'&source='+source).success(function(data){
						...
						scope.loadCfList = data.cfList;
						for(var i= 0,len= scope.loadCfList.length;i<len;i++){
							//解析status
							scope.loadCfList[i].status=parseStatus(scope.loadCfList[i].status);
							// 解析source和sourceId
							var sourceInfo= parseSourceInfo(scope.loadCfList[i].source, scope.loadCfList[i].sourceId);
							scope.loadCfList[i].sourceUrl=sourceInfo.sourceUrl;
							scope.loadCfList[i].sourceName=sourceInfo.sourceName;
						}
						scope.cfList = scope.cfList.concat(scope.loadCfList);
	...
	return {
        ...
        link: function (scope, element, attrs) {
        	element.bind("focusout", function(event) {
        		...
        		var req = {
       				 method: 'put',
       				 url: pre_address+'/api/site/user/list/updateDesc',
       				 params: { desc: desc, id: list.id}
       		    }
        			...
        			var modalInstance = comfirm($modal);
	           		 modalInstance.result.then(function (callBack) {
	           			 if(callBack.comfirm){
	           				var req = {
	                 				 method: 'put',
	                 				 url: pre_address+'/api/site/user/company/updateHeart',
	...
	return {
        ...
        link: function (scope, element, attrs) {
        	element.bind("click", function(event) {
        		...
        		var req = {
        				 method: 'put',
        				 url: pre_address+'/api/site/user/list/create',
    ...
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
        	...
        	if(val == '' || val ==null) return;
				...
				scope.searchList = [];
					//this request can make result show right now though the reponse is nothing
				$http.get(pre_address+'/api/site/user/list/search').success(function(data){
					var i=0;
				    angular.forEach(scope.allList.companyListVOList, function(value, key){
				    		if(angular.lowercase(value.companyList.name).indexOf(angular.lowercase(val)) > -1 && i <8){
				    			this.push(value);
				    			i++;
				    		}
					}, scope.searchList)
        	 ...
        	 if(event.which === 13) {  
                 scope.$apply(function(){
                	...
                	if(flag){
                		...
                		var selectedList = {};
                		angular.forEach(scope.allList.companyListVOList, function(v, key){
        	        		if(v.companyList.id == listId){
        	        			selectedList = v;
        	        		}
        				})
...
//                		el.addClass('bg-selected');
                		...
                		var i = 0 ;
                		angular.forEach(scope.allList.companyListVOList, function(v, key){
        	        		if(v.companyList.name.indexOf(value) > -1 && i<8){
...
function cfStatus($rootScope, $window, $http, crowdfundingService, cfTotalService){
	return {
		restrict: 'A',
		link: function (scope, element, attrs) {
			element.bind("click", function(event) {
				...
				$window.location.href = "#/index/crowdfunding/"+model+"/"+source;
...
function cfSource($rootScope, $window,$http, crowdfundingService, cfTotalService){
	return {
		restrict: 'A',
		link: function (scope, element, attrs) {
			element.bind("click", function(event) {
				...
				$window.location.href = "#/index/crowdfunding/"+status+"/"+model;
				//$http.get(pre_address+'/api/site/crowdfunding/getAll?page=1&source='+model+'&status='+status).success(function(data){
